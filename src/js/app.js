...
import {
  initMainView,
  setMainView,
  registerAccountSettingsRenderer,
} from "./main-view.js";
import {
  initAccountSettings,
  renderAccountSettings,
} from "./account-settings-view.js";
...
async function caricaProfiloUtente() {
  ...
  document
    .querySelectorAll('[data-get-info="username-fullname"]')
    .forEach((el) => {
  ...
  if (nomeCompleto)
    nomeCompleto.textContent = `${profilo.nome} ${profilo.cognome}`;
...
async function syncAdminDashboardLink(profilo) {
  ...
  if (error) {
    console.error(
      "Impossibile verificare i permessi di amministratore",
      error.message,
    );
    return;
  }
